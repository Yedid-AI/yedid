  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Cree le {new Date(agent.created_at).toLocaleDateString('fr-FR')}
          </p>
        </div>
        <Badge variant={agent.is_active ? 'default' : 'secondary'} className="ml-auto">
      ...
      <Tabs defaultValue="config">
        <TabsList>
          <TabsTrigger value="config"><Settings className="mr-1.5 h-4 w-4" />Config</TabsTrigger>
          <TabsTrigger value="playbooks"><BookOpen className="mr-1.5 h-4 w-4" />Playbooks</TabsTrigger>
          <TabsTrigger value="tools"><Wrench className="mr-1.5 h-4 w-4" />Tools</TabsTrigger>
          <TabsTrigger value="escalation"><Zap className="mr-1.5 h-4 w-4" />Escalation</TabsTrigger>
        </TabsList>
