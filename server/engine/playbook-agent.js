 ...
 */
export async function runPlaybookAgent({ agentConfig, playbook, agentTools = [], userMessage, conversationHistory, supabase, userId, enterpriseUserId, contactContext, sessionId, conversationId }) {
        ...
        if (toolCall.name === 'search_knowledge_base') {
          ...
          if (matchedTool?.type === 'internal') {
            ...
            if (toolCallCache.has(sig)) {
              ...
              toolResult = await executeInternalTool(matchedTool.handler, toolCall.arguments, { supabase, userId, sessionId, conversationId, enterpriseUserId })
